...
import { ProjectHandler, type GetProjectRouterRequest } from './handlers/project';
...
router.get("/api/projects/status", async (request: IRequest, _env, _ctx, container) => {
...
router.get("/api/projects/:id/award", async (request: IRequest, _env, _ctx, container) => {
  const projectService = container.getProjectService();
