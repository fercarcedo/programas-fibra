...
def awarded_areas(
    awarded_projects_file_path: str, entities_file_path: str, program_name: str
) -> FeatureCollection:
    ...
    cols_to_ffill = [
        ...
        if not excel_df[col].dropna().empty
        and excel_df[col].dropna().iloc[0]
        not in ["Código zona de excepción", "Nombre zona de excepción"]
    ]
    ...
    projects_column = (
        PROJECTS_COLUMN_INDEX if not is_old_peba else PROJECTS_COLUMN_INDEX_OLD
    )
    ...
    if not is_old_peba:
        ...
        grantees = (
            excel_projects_df[GRANTEES_COLUMN_INDEX_OLD].ffill().dropna().iloc[1:]
        )

    code_to_coordinates = ine_code_to_coordinates(entities_file_path)
    project_to_grantee = dict(zip(projects, grantees))

    return map_awarded_areas(
        excel_df, project_to_grantee, code_to_coordinates, program_name, is_old_peba
    )
...
def map_awarded_areas(
    ...
    for row in df.itertuples(name=None):
        ...
        properties = create_properties(
            row, project_to_grantee, program_name, is_old_peba
        )
...
def create_properties(
    ...
    return AreaPropertiesTown(
        ...
        exception_zone_code=(
            None
            if exception_code_value is None
            or (
                isinstance(exception_code_value, float)
                and math.isnan(exception_code_value)
            )
            else exception_code_value
        ),
        exception_zone_name=(
            None
            if exception_name_value is None
            or (
                isinstance(exception_name_value, float)
                and math.isnan(exception_name_value)
            )
            else exception_name_value
        ),
...
def to_geojson(properties: AreaProperties, coordinates: tuple[float, float]) -> Feature:
    ...
    return Feature(
        geometry=point, properties={**properties.__dict__, "type": str(properties.type)}
    )
...
def execute(
    ...
    areas_geojson = awarded_areas(
        awarded_projects_file_path, entities_file_path, program_name
    )
