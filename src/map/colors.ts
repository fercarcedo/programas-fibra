...
const NAME_TO_OPERATOR: Record<string, string> = {
  ...
  "R CABLE Y TELECOMUNICACIONES GALICIA, S.A.": "EUSKALTEL",
  "R CABLE Y TELECABLE TELECOMUNICACIONES, S.A.": "EUSKALTEL",
...
export const getProgramColor = (
  ...
  const mappedProgramName = programName.toUpperCase().replace(" ", "_");
...
export const getOperatorColors = (): Record<
  string,
...
export const getProgramColors = (): Record<
  string,
  [number, number, number]
> => {
...
export const getOperatorNames = (operator: string): string[] => {
  return Object.entries(NAME_TO_OPERATOR).reduce<string[]>(
    (acc, [key, value]) => {
      if (value === operator) {
        acc.push(key);
      }
